...
def test_worked_contrast_between_pc_related_and_ordinary_diabetes():
    ...
    one is type 2."""
    pc_like = endpac_predict(glucose_at_diabetes_mg_dl=180,
                             glucose_one_year_before_mg_dl=95,
                             weight_change_kg=-7.0, age_at_diabetes_onset=72)
    t2_like = endpac_predict(glucose_at_diabetes_mg_dl=130,
                             glucose_one_year_before_mg_dl=118,
                             weight_change_kg=5.0, age_at_diabetes_onset=52)
...
def test_glucose_term_is_a_category_difference_not_mg_dl():
    ...
    out = endpac_predict(glucose_at_diabetes_mg_dl=130,
                         glucose_one_year_before_mg_dl=95,
...
def test_risk_bands():
    ...
    assert endpac_predict(glucose_at_diabetes_mg_dl=130, weight_change_kg=0.0,
                          **kw)["risk_band"] == "high"          # score 3
    assert endpac_predict(glucose_at_diabetes_mg_dl=130, weight_change_kg=5.0,
                          **kw)["risk_band"] == "very_low"      # score -1


def test_metadata():
    out = endpac_predict(glucose_at_diabetes_mg_dl=150,
                         glucose_one_year_before_mg_dl=100,
                         weight_change_kg=-3.0, age_at_diabetes_onset=65)
...
def test_out_of_scope_glucose_pairs_are_rejected():
    ...
    meaningless number.
    """
    ok = dict(glucose_at_diabetes_mg_dl=150, glucose_one_year_before_mg_dl=100,
              weight_change_kg=-3.0, age_at_diabetes_onset=65)
...
def test_glucose_term_stays_within_the_published_1_to_4_range():
    for before in (95, 105, 120):
        for at in (130, 200):
            out = endpac_predict(glucose_at_diabetes_mg_dl=at,
