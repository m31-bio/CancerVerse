...
def test_categorise_reference_profile_is_all_zero():
    cats = categorise(
        age=50, race="white", n_biopsies=0, atypical_hyperplasia="unknown",
        age_menarche=14, age_first_birth=19, n_relatives=0,
    )
...
def test_relative_risk_formula_matches_published_linear_predictor():
    ...
    cats = categorise(
        age=45, race="white", n_biopsies=1, atypical_hyperplasia="yes",
        age_menarche=11, age_first_birth=32, n_relatives=2,
    )
    beta = C.BETA["white"]
    expected_lp1 = (
...
def test_lowest_risk_profile_10y_risk_is_low_single_digit_percent():
    out = bcrat_predict(
        start_age=50, end_age=60, race="white", n_biopsies=0,
        atypical_hyperplasia="unknown", age_menarche=14, age_first_birth=19,
        n_relatives=0,
    )
...
def test_high_risk_profile_exceeds_average_of_same_age():
    high = bcrat_predict(
        start_age=45, end_age=55, race="white", n_biopsies=1,
        atypical_hyperplasia="yes", age_menarche=11, age_first_birth=32,
        n_relatives=2,
    )
    avg = bcrat_predict(
        start_age=45, end_age=55, race="white", n_biopsies=0,
        atypical_hyperplasia="unknown", age_menarche=14, age_first_birth=19,
        n_relatives=0,
    )
...
def test_lifetime_projection_matches_seer_order_of_magnitude():
    out = bcrat_predict(
        start_age=20, end_age=90, race="white", n_biopsies=0,
        atypical_hyperplasia="unknown", age_menarche=14, age_first_birth=19,
        n_relatives=0,
    )
...
def test_end_age_must_exceed_start_age():
    with pytest.raises(ValueError, match="end_age"):
        bcrat_predict(
            start_age=55, end_age=45, race="white", n_biopsies=0,
            atypical_hyperplasia="unknown", age_menarche=14,
            age_first_birth=19, n_relatives=0,
        )
...
def test_rejects_ages_outside_hazard_table_range():
    with pytest.raises(ValueError, match="20"):
        bcrat_predict(
            start_age=10, end_age=20, race="white", n_biopsies=0,
            atypical_hyperplasia="unknown", age_menarche=14,
            age_first_birth=19, n_relatives=0,
        )
