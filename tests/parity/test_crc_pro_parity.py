...
Kattan MW, Cooper GS, Jackson L, Koroukian S. J Am Board Fam Med. 2014;27(1):42-55.
...
NSAID = {"No": "no", "Yes, but not currently": "previously", "Yes, currently": "currently"}
ESTROGEN = {"No": "no", "Yes, but not currently": "previously", "Yes-currently": "currently"}
...
def _kwargs(c: dict) -> dict:
    return dict(
        male=c["male"], age=c["age"], ethnicity=c["ethnicity"],
        weight_lb=c["weight_lb"], height_in=c["height_in"],
        years_education=c["years_education"], pack_years=c["pack_years"],
        alcohol_drinks_per_day=c["alcohol"], family_history=c["family_history"],
        multivitamin=c["multivitamin"], diabetes=c["diabetes"],
        estrogen=ESTROGEN[c["estrogen"]], nsaid=NSAID[c["nsaid"]],
        aspirin=ASPIRIN[c["aspirin"]], red_meat_oz_per_day=c["red_meat"],
...
def test_previous_estrogen_is_scored_as_no_estrogen_by_the_deployed_tool():
    ...
    base = dict(male=False, age=65, ethnicity="white", weight_lb=160,
                height_in=65, years_education=14, pack_years=5,
                alcohol_drinks_per_day=1.0, family_history=False,
                multivitamin=False, diabetes=False, nsaid="no")
    none_ = crc_pro_predict(**base, estrogen="no", emulate_deployed_defect=True)["risk"]
    prev = crc_pro_predict(**base, estrogen="previously", emulate_deployed_defect=True)["risk"]
...
def test_men_and_women_are_different_models_not_one_with_a_sex_term():
    ...
    common = dict(age=65, ethnicity="white", weight_lb=170, height_in=68,
                  years_education=14, pack_years=10,
                  alcohol_drinks_per_day=1.0, family_history=False,
                  multivitamin=False, diabetes=False)
    m1 = crc_pro_predict(male=True, **common, estrogen="currently", nsaid="currently")["risk"]
    ...
    f1 = crc_pro_predict(male=False, **common, aspirin="currently",
                         red_meat_oz_per_day=4.0, activity_hours_per_day=3.0)["risk"]
    f2 = crc_pro_predict(male=False, **common, aspirin="no",
                         red_meat_oz_per_day=0.0, activity_hours_per_day=0.0)["risk"]
    assert f1 == pytest.approx(f2), "aspirin, meat and activity must not enter the women's model"
...
def test_the_fixture_covers_every_categorical_level_and_both_bound_ends():
    ...
    assert {ESTROGEN[c["estrogen"]] for c in cases if not c["male"]} == set(ESTROGEN_BETA)
    ...
    for field, key in (("age", "age"), ("weight_lb", "weight_lb"),
                       ("height_in", "height_in"), ("pack_years", "pack_years"),
...
def test_bmi_is_derived_from_pounds_and_inches():
    ...
    out = crc_pro_predict(male=True, age=60, ethnicity="white", weight_lb=lb,
                          height_in=inches, years_education=14, pack_years=0,
                          alcohol_drinks_per_day=0.0, family_history=False,
                          multivitamin=False, diabetes=False)
    assert out["bmi"] == pytest.approx(25.0)


def test_out_of_scope_inputs_are_refused():
    ok = dict(male=True, age=60, ethnicity="white", weight_lb=170, height_in=68,
              years_education=14, pack_years=5, alcohol_drinks_per_day=1.0,
              family_history=False, multivitamin=False, diabetes=False)
    for field, bad in (("age", 40), ("age", 90), ("weight_lb", 60),
                       ("height_in", 55), ("pack_years", 60),
                       ("alcohol_drinks_per_day", 15), ("years_education", 3)):
...
def test_metadata():
    out = crc_pro_predict(male=False, age=60, ethnicity="black", weight_lb=150,
                          height_in=64, years_education=12, pack_years=0,
                          alcohol_drinks_per_day=0.0, family_history=False,
                          multivitamin=False, diabetes=False)
