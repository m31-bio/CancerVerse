...
def test_hpv_positive_light_smoker_any_n_stage_is_low_risk():
    out = ang2010_rpa_predict(
        hpv_positive=True, pack_years=5, n_stage="N3", t_stage="T2"
    )
...
def test_hpv_positive_heavy_smoker_low_n_stage_is_low_risk():
    out = ang2010_rpa_predict(
        hpv_positive=True, pack_years=20, n_stage="N2a", t_stage="T3"
    )
    assert out["risk_group"] == "low"
...
def test_hpv_positive_heavy_smoker_high_n_stage_is_intermediate():
    for n in ("N2b", "N2c", "N3"):
        out = ang2010_rpa_predict(
            hpv_positive=True, pack_years=20, n_stage=n, t_stage="T2"
        )
...
def test_hpv_negative_light_smoker_t2_or_t3_is_intermediate():
    for t in ("T2", "T3"):
        out = ang2010_rpa_predict(
            hpv_positive=False, pack_years=0, n_stage="N0", t_stage=t
        )
...
def test_hpv_negative_heavy_smoker_is_high_regardless_of_t_stage():
    out = ang2010_rpa_predict(
        hpv_positive=False, pack_years=15, n_stage="N0", t_stage="T2"
    )
...
def test_pack_years_boundary_is_at_10():
    ...
    assert (
        ang2010_rpa_predict(pack_years=10.01, **common)["risk_group"] == "intermediate"
    )


def test_stage_normalization_accepts_clinical_prefix_and_case():
    a = ang2010_rpa_predict(
        hpv_positive=True, pack_years=20, n_stage="cN2b", t_stage="T2"
    )
    b = ang2010_rpa_predict(
        hpv_positive=True, pack_years=20, n_stage="n2b", t_stage="t2"
    )
    assert a["risk_group"] == b["risk_group"] == "intermediate"


def test_published_survival_and_hazard_ratios_attached():
    low = ang2010_rpa_predict(
        hpv_positive=True, pack_years=0, n_stage="N0", t_stage="T2"
    )
    inter = ang2010_rpa_predict(
        hpv_positive=True, pack_years=20, n_stage="N2b", t_stage="T2"
    )
    high = ang2010_rpa_predict(
        hpv_positive=False, pack_years=20, n_stage="N0", t_stage="T2"
    )
...
def test_invalid_inputs():
    with pytest.raises(ValueError, match="pack_years"):
        ang2010_rpa_predict(
            hpv_positive=True, pack_years=-1, n_stage="N0", t_stage="T2"
        )
    with pytest.raises(ValueError, match="n_stage"):
        ang2010_rpa_predict(
            hpv_positive=True, pack_years=5, n_stage="bogus", t_stage="T2"
        )
    with pytest.raises(ValueError, match="t_stage"):
        ang2010_rpa_predict(
            hpv_positive=True, pack_years=5, n_stage="N0", t_stage="bogus"
        )


def test_metadata():
    out = ang2010_rpa_predict(
        hpv_positive=True, pack_years=5, n_stage="N0", t_stage="T2"
    )
...
def test_default_definition_is_the_literal_primary_reading():
    out = ang2010_rpa_predict(
        hpv_positive=False, pack_years=0, n_stage="N0", t_stage="T1"
    )
