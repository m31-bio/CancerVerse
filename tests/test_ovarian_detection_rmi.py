...
def test_rmi1_worked_example():
    ...
    out = rmi_predict(
        ultrasound_score=3, postmenopausal=True, ca125=35.0, variant="rmi1"
    )
...
def test_rmi1_zero_ultrasound_score_forces_index_to_zero():
    ...
    out = rmi_predict(
        ultrasound_score=0, postmenopausal=True, ca125=1000.0, variant="rmi1"
    )
...
def test_rmi3_removes_the_zero():
    ...
    out = rmi_predict(
        ultrasound_score=0, postmenopausal=True, ca125=1000.0, variant="rmi3"
    )
...
def test_rmi4_requires_max_diameter():
    with pytest.raises(ValueError, match="max_diameter_cm"):
        rmi_predict(
            ultrasound_score=2, postmenopausal=True, ca125=100.0, variant="rmi4"
        )
...
def test_premenopausal_scores_lower_than_postmenopausal():
    pre = rmi_predict(
        ultrasound_score=3, postmenopausal=False, ca125=50.0, variant="rmi1"
    )
    post = rmi_predict(
        ultrasound_score=3, postmenopausal=True, ca125=50.0, variant="rmi1"
    )
