...
RESOLVED 2026-08-18: an earlier probe looked like a disagreement, and did not reproduce
------------------------------------------------------------------------------------
CUHK's Medical Data Analytics Centre, an aMAP collaborating site, hosts a
calculator at https://mdac.cuhk.edu.hk/calculators/amap/ . An early probe of
three patients found its output sitting 2-3 points above this module's
published-formula output (e.g. 53.82 here vs 56 there for a 50-year-old male
with bilirubin 15, albumin 42, platelets 200), and this docstring used to
record that as an unresolved conflict, with `parity_status` deliberately left
at `not_checked` rather than forced to agree.

Re-probing those exact three patients later returned the PUBLISHED values
(54/41/65, not 56/44/68), the discrepancy did not reproduce, most likely an
artefact of the earlier probe rather than a real formula difference. A wider
re-check across 15 patients spanning both sexes, all three risk bands and the
full analyte range now matches the calculator's integer score on 15/15 and its
risk band on 15/15. `parity_status` is `checked` accordingly; see the
registry's parity_note for the fixture and test. Every slope this module
computes still agrees with the paper's own reported coefficients (age ~0.406
pts/yr, male ~6.0, albumin -0.267 pts/(g/L), platelets -0.0656), which was
never in question.
